...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                             ...
                             QDateEdit, QCalendarWidget, QStyleFactory, QCheckBox)
...
class FormularioFuncionario(QWidget):
    ...
    def initUI(self):
        ...
        self.sexo_combo.addItems(["Masculino", "Feminino"])
        ...
        # Título da seção de Acesso ao Sistema
        acesso_titulo = QLabel("Acesso ao Sistema")
        acesso_titulo.setFont(QFont("Arial", 14, QFont.Bold))
        acesso_titulo.setStyleSheet("color: white; margin-top: 20px;")
        acesso_titulo.setAlignment(Qt.AlignCenter)

        # Campo Nome de Usuário
        self.usuario_label = QLabel("Nome de Usuário:")
        self.usuario_label.setStyleSheet(label_style)
        self.usuario_input = QLineEdit()
        self.usuario_input.setStyleSheet(input_style)
        self.usuario_input.setMaximumHeight(30)
        self.usuario_input.setFixedWidth(250)

        usuario_layout = QFormLayout()
        usuario_layout.setLabelAlignment(Qt.AlignRight | Qt.AlignVCenter)
        usuario_layout.setFormAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        usuario_layout.addRow(self.usuario_label, self.usuario_input)

        # Campo Senha (apenas se for criar usuário junto com funcionário)
        self.senha_label = QLabel("Senha:")
        self.senha_label.setStyleSheet(label_style)
        self.senha_input = QLineEdit()
        self.senha_input.setStyleSheet(input_style)
        self.senha_input.setEchoMode(QLineEdit.Password)  # Oculta a senha
        self.senha_input.setMaximumHeight(30)
        self.senha_input.setFixedWidth(250)

        senha_layout = QFormLayout()
        senha_layout.setLabelAlignment(Qt.AlignRight | Qt.AlignVCenter)
        senha_layout.setFormAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        senha_layout.addRow(self.senha_label, self.senha_input)

        # Adicionar seção de acesso ao layout principal
        main_layout.addWidget(acesso_titulo)
        main_layout.addLayout(usuario_layout)
        main_layout.addLayout(senha_layout)

        # Adicionar checkbox para controlar se deve criar usuário
        self.criar_usuario_check = QCheckBox("Criar acesso ao sistema")
        self.criar_usuario_check.setStyleSheet("color: white; font-size: 14px;")
        self.criar_usuario_check.setChecked(False)
        self.criar_usuario_check.toggled.connect(self.alternar_campos_usuario)
        main_layout.addWidget(self.criar_usuario_check)
        ...
        # Desabilitar campos de usuário inicialmente
    ...
    def alternar_campos_usuario(self):
        """Alterna a visibilidade dos campos de usuário e senha"""
        is_checked = self.criar_usuario_check.isChecked()
        self.usuario_input.setEnabled(is_checked)
        self.senha_input.setEnabled(is_checked)
    ...
    def salvar_funcionario(self):
        ...
        # Obter dados de usuário se o checkbox estiver marcado
        criar_usuario = self.criar_usuario_check.isChecked()
        nome_usuario = self.usuario_input.text() if criar_usuario else ""
        senha = self.senha_input.text() if criar_usuario else ""
        ...
        if criar_usuario:
            ...
            from base.banco import verificar_usuario_existente
            if verificar_usuario_existente(nome_usuario):
                self.mostrar_mensagem("Erro", 
                                f"O nome de usuário '{nome_usuario}' já está em uso.", 
                                QMessageBox.Warning)
                self.usuario_input.setFocus()
                return
